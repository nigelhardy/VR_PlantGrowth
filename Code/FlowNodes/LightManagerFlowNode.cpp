...
public:
	CLFlowNodeRegistrator() {}
	~CLFlowNodeRegistrator()
	{

	}
};
CLFlowNodeRegistrator g_myFlowNodeRegistrator;

REGISTER_FLOW_NODE("entity:LightTracker", CFlowNode_LightManager);
CFlowNode_LightManager::CFlowNode_LightManager(SActivationInfo* pActInfo)
{
...
IFlowNodePtr CFlowNode_LightManager::Clone(SActivationInfo *pActInfo)
{
	return new CFlowNode_LightManager(pActInfo);
};

void  CFlowNode_LightManager::GetMemoryUsage(ICrySizer* pSizer) const
{
	pSizer->AddObject(this, sizeof(*this));
}

void CFlowNode_LightManager::GetConfiguration(SFlowNodeConfig& config)
{
	// name - Do not use the underscore character '_' in port names

	static const SInputPortConfig in_config[] = {
		InputPortConfig<EntityId>("LightEntityId", _HELP("Light Entity Id"),0),
...
void CFlowNode_LightManager::ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
{
	switch (event)
	{
		...
		if (IsPortActive(pActInfo, eInputPort_LightEntityId))
		{
			...
			if (pEntity != NULL && value != NULL)
			{
				...
				if (x != NULL)
				{
					...
					//logVector("Light", gEnv->pEntitySystem->GetEntity(*value)->GetPos());
				}
			}
